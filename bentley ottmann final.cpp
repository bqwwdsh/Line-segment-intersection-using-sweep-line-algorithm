...
map<pair<double,double>, pair<pair<double,double> ,int>>horizontal_start_point, horizontal_end_point;
vector<Segment>ans[305];
vector<Segment>up_segments[305];
Segment segments[305];
int ans_cnt;
...
bool is_any_horizontal_now;
Segment cur_horizontal_segment ;
...
void draw_lines_and_end_points()
{
    glColor3f(0,1,0);

    for(int i = 1; i <=n; i++){
        Segment s = segments[i];
        Point a = s.start_point;
        Point b = s.end_point;
        ...
        glPushMatrix();
        {
            ...
            glBegin(GL_LINES);
            {
                glVertex3d( a.x,  a.y, 3.0);
                glVertex3d( b.x,  b.y, 3.0);
            }
            glEnd();
        }
        glPopMatrix();
...
void draw_intersections()
{
    ...
    for(it = intersection_map.begin(); it != intersection_map.end(); it++){
        ...
        //glVertex3d( point.first,  point.second, 3.0);
        glPushMatrix();
        {
            glTranslatef(point.first,point.second,3.0);
            glScalef(0.5,0.5,1);
            drawSquare();
        }
        glPopMatrix();
...
void handle_event_point(Event e)
{
    ...
    if(e.event_type == HORIZONTAL_START){
        Point a = p;
        pair<pair<double,double>,int> u = horizontal_end_point[{p.x,p.y}];
        Point b = Point(u.first.first, u.first.second);
        cur_horizontal_segment = Segment(a,b,u.second);
        is_any_horizontal_now = 1;
    }

    multiset<Segment>::iterator it;
    vector<Segment>Up,Lp,Cp;

    for(Segment s : up_segments[Map[{p.x,p.y}]]){
        Up.push_back(s);
    }
    ...
    while(s != left_dummy_segment){
        ...
        /// jodi kono horizontal segment akhn thake
        ...
        if(fabs(s.start_point.y - s.end_point.y) < epsilon  ){
            ...
            if(is_any_horizontal_now){
                if(x - cur_horizontal_segment.start_point.x > epsilon && x - cur_horizontal_segment.end_point.x < epsilon){
                    ...
                    intersection_segments[cnt].insert(cur_horizontal_segment.segment_num);
    ...
    if(e.event_type == HORIZONTAL_END) is_any_horizontal_now = 0;
...
void input_plus_others()
{
    ...
    for(int i = 0; i < n; i++){
        ...
        if(fabs(a.y - b.y) < epsilon){
            ...
            horizontal_end_point[{a.x,a.y}] = {{b.x,b.y},i+1};
        }
        else{
